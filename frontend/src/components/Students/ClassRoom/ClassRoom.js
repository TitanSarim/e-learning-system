...
import {
  ...
  clearErrors,
} from "../../../actions/InrolledCourseAction";
...
const ClassRoom = () => {
  ...
  const { InrolledCourse, loading, error } = useSelector(
    (state) => state.userInrolledCourses
  );
  ...
  const handleSelectedIndex = (content, index) => {
    let ActivevideosArray =
      JSON.parse(localStorage.getItem("selectedWeekIndex")) || [];

    const existingVideoIndex = ActivevideosArray?.findIndex(
      (video) => video.slug === courseDetails?.slug
    );
    ...
    if (existingVideoIndex !== -1) {
      ActivevideosArray[existingVideoIndex].weekIndex =
        newVideoObject.weekIndex;
      ...
      ActivevideosArray[existingVideoIndex].videoTitle =
        newVideoObject.videoTitle;
    } else {
    ...
    localStorage.setItem(
      "selectedWeekIndex",
      JSON.stringify(ActivevideosArray)
    );
  ...
  useEffect(() => {
    const weekIndexArray = JSON.parse(
      localStorage.getItem("selectedWeekIndex")
    );
    let isMounted = true;

    if (weekIndexArray && courseContent?.length > 0 && isMounted) {
      const weekIndex = weekIndexArray?.find(
        (item) => item.slug === courseDetails?.slug
      );
      if (weekIndex) {
  ...
  return (
    <div className="class-container">
      ...
      {loading ? (
        <div className="class-container-loader">
          <Loader />
        </div>
      ) : (
        <div className="class-container-1">
          <div className="class-main-container">
                  ...
                  <div className="class-description-section-content-container">
                    {courseContent?.map((content, index) => {
                      ...
                      const allVideosCompleted =
                        weekData &&
                        weekData.videos.every((video) => video.completed);

                      return (
                        <div
                          className="class-description-section-content"
                          key={index}
                        >
                          ...
                          <div>
                            ...
                            <span>
                              {allVideosCompleted ? "Complete" : "Incomplete"}
                            </span>
                          </div>
                          <button
                            onClick={() => handleSelectedIndex(content, index)}
                          >
                            Take Lecture
                          </button>
                        </div>
            ...
            <div className="class-video-container-1">
              <VideoSection
                selectVideToPlay={selectVideoToPlay}
                courseDetails={courseDetails}
                setVideoPercentage={setVideoPercentage}
                videoPercentage={videoPercentage}
                selectWeekIndex={selectWeekIndex}
                setCouseCompletion={setCouseCompletion}
              />
            </div>
            ...
            <div className="class-course-container-1">
              <CourseChat
                ...
                courseDetails={courseDetails}
                setSelectVideoToPlay={setSelectVideoToPlay}
                videoPercentage={videoPercentage}
                couseCompletion={couseCompletion}
              />
            </div>
          </div>
        </div>
