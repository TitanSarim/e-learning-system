...
const CourseDetailReviews = ({IsEnrolled, comments, courseSlug}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const averageRating = ((ratingOne + ratingTwo + ratingThree) / 3).toFixed(2);
  ...
  return (
    <div className='CourseDetailReviews'>
     
        {
            isCommented === true ? (
              ""
                ...
                IsEnrolled === true ? (
                    <button onClick={() => hanldeReviewModelOpen()}>Give Your Feedback</button>
                ) : null
            )
        }
      ...
      : (
        <div className='CourseDetailReviews-all-feedbacks'> 
          {AllComments?.data.map((data) => (
            <div key={data.user} className='CourseDetailReviews-all-feedbacks-wrapper'>
      ...
      <Popup open={feedBackDialogeOpen} closeOnDocumentClick onClose={() => setFeedBackDialogeOpen(false)} className='course-add-feedback-dialogue-popup'>

          <div className='course-add-feedback-container'>

            <p>Add Your Valuable Feedback</p>

            <form className='course-add-feedback-wrapper' onSubmit={(e) => handleSubmit(e)}>
              ...
              <div className='course-add-feedback-comment'>
                <p>Add Comment:</p>
                <textarea required value={addComment} onChange={(e) => setAddComment(e.target.value)}/>
              </div>

              <div className='course-add-feedback-button'>
                <input type='Submit' value="Post" />
                <button onClick={() => setFeedBackDialogeOpen(false)}>Cancel</button>
              </div>
